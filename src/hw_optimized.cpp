...
// single PE
void PE(int d, uint64_t R[MAX_EDIT_THRESHOLD + 1], uint64_t curBitmask, uint64_t tracebackMatrix[MAX_TEXT_SZ][MAX_PATTERN_SZ + 1][4], int i, uint64_t oldR_prev[MAX_EDIT_THRESHOLD + 1], uint64_t oldR_curr[MAX_EDIT_THRESHOLD + 1])
{
#pragma HLS FUNCTION_INSTANTIATE variable = d

  uint64_t deletion = oldR_prev[d];
  uint64_t substitution = deletion << 1;
  uint64_t insertion = R[d - 1] << 1;
  uint64_t match = (oldR_curr[d] << 1) | curBitmask;
...
#pragma HLS ARRAY_PARTITION variable = PARAMS dim = 1 complete
...
#pragma HLS ARRAY_PARTITION variable = tracebackMatrix dim = 2 complete
  // reshape for more bits per bram access
#pragma HLS ARRAY_RESHAPE variable = tracebackMatrix dim = 3 complete
...
#pragma HLS ARRAY_PARTITION variable = patternBitmasks dim = 1 complete
...
#pragma HLS LOOP_TRIPCOUNT max = 64
    // UNROLL with conditional exec (if (i < m)) costly here, so just pipeline
#pragma HLS PIPELINE
  ...
  uint64_t oldR_prev[MAX_EDIT_THRESHOLD + 1];
  uint64_t oldR_curr[MAX_EDIT_THRESHOLD + 1];
  uint64_t substitution, insertion, match, deletion, curBitmask;

#pragma HLS ARRAY_PARTITION variable = R dim = 1 complete
#pragma HLS ARRAY_PARTITION variable = oldR_prev dim = 1 complete
#pragma HLS ARRAY_PARTITION variable = oldR_curr dim = 1 complete

LOOP_R_INIT:
  for (int i = 0; i < MAX_EDIT_THRESHOLD + 1; i++)
  {
#pragma HLS UNROLL
    R[i] = max;
    oldR_prev[i] = max;
...
#pragma HLS PIPELINE II = 1
    ...
    LOOP_EDIT_DIST:
      for (int d = 1; d <= MAX_EDIT_THRESHOLD; d++)
      {
#pragma HLS UNROLL
        if (k >= d)
          PE(d, R, curBitmask, tracebackMatrix, i, oldR_prev, oldR_curr);
      }
    }
  }
...
#pragma HLS LOOP_TRIPCOUNT max = 65
    // UNROLL with conditional exec (if (t < k)) costly here, so just pipeline
#pragma HLS PIPELINE
...
#pragma HLS ARRAY_PARTITION variable = output_strings dim = 1 complete
